...
DDIM 常被概括成“把 DDPM 的采样步数从 1000 步降到几十步”。这个结论没错，但不够解释它为什么成立。DDIM 的关键不在少跑几次网络，而在于它重新写了逆过程的约束条件：DDPM 那条逐步、带随机性的马尔可夫逆链，并不是唯一可行的生成路径。
...
DDIM 的意义不只是减少步数，而是把随机逆扩散改写成了更可控的更新过程。这层改写，也是后面很多连续时间扩散方法继续往下推的基础。
